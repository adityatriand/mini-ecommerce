...
func main() {
	...
	log, err := logger.NewLogger(loggerConfig, "product-service")
	if err != nil {
	...
	productService := service.NewProductService(productRepo, redisCache, log.GetZapLogger())
	productHandler := handler.NewProductHandler(productService, log.GetZapLogger())
	...
	r.Use(gin.Recovery())
	r.Use(gin.Logger())
	r.Use(metrics.PrometheusMiddleware("product-service"))
	...
	log.Info("Starting product service", zap.String("port", port))
	...
	<-quit
	log.Info("Product service shutting down gracefully...")
}
